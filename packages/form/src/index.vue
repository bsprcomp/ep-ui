      ...
      >
        ...
        <template v-if="item.isSelfCom">
          <component
            ...
            v-on="cEvent(item, 't-selecEPTable')"
          />
          <component
            v-else
            :is="item.comp"
...
// 下拉选择表格组件 ref
const tselecEPTableref: any = ref({})
// 下拉选择表格组件 动态ref
const handleRef = (el: any, key: any) => {
  if (el) {
    tselecEPTableref.value[`tselecEPTableref-${key}`] = el
  }
}
const selfResetFields = () => {
  // 获取所有下拉选择表格组件
  const refList = Object.keys(tselecEPTableref.value).filter(item =>
    item.includes("tselecEPTableref")
  )
  if (refList.length > 0 && tselecEPTableref.value) {
    refList.map(val => {
      // console.log('9999', val)
      tselecEPTableref.value[val].clear()
    })
  }
